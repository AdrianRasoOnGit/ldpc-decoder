...
public final class OffsetMinSumDecoder implements LdpcDecoder {
    ...
    public OffsetMinSumDecoder(CsrMatrix h, int maxIterations, float beta) {
        if (beta < 0.0f) {
    ...
    private void checkToVariable() {
        for (int row = 0; row < h.rows(); row++) {
            ...
            for (int target = start; target < end; target++) {
                ...
                float magnitude = Math.max(minAbs - beta, 0.0f);
                r[target] = sign * magnitude;
            }
        }
    }
